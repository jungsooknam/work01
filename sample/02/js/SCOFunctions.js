...
//	page_count = page_sum;
   ...
   if(doGetValue("cmi.objectives._count")>0){
		...
		doSetValue("cmi.objectives._count",page_count);		

		for(i=0;i<page_count;i++){

		doSetValue("cmi.objectives."+i+".id",i);

		doSetValue("cmi.objectives."+i+".completion_status","incomplete");
...
function PageLocation(page,count){

	 doSetValue("cmi.location",page);

	 doSetValue("cmi.objectives."+count+".completion_status","completed");



     var page_count = doGetValue('cmi.objectives._count');
	 ...
	 for(i=0;i<page_count;i++){
		...
		if("incomplete" == doGetValue("cmi.objectives."+i+".completion_status")){
	...
	var new_progress = progress_count / page_count;
	

	var old_progress = doGetValue("cmi.progress_measure");

	if(old_progress == "") old_progress=0;

	if(new_progress>old_progress){

		doSetValue("cmi.progress_measure",progress_count / page_count);

	}
	var page_progress = progress_count+"/"+page_count;
	doSetValue("cmi.suspend_data",page_progress);
